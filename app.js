...
(() => {
  ...
  function updateButtonStates() {
    btnStart.disabled = !(stream && cvReady) || tracking || trackingArmed;
  }
  ...
  btnShare.addEventListener('click', async () => {
    try {
      ...
      setStatus('Screen shared', 'warn');
      log('Screen share started.');

      // Resize canvases once we know native video size
      ...
      log(`Screen share failed: ${escapeHtml(err.message || String(err))}`);
  ...
  function stopTracking() {
    ...
    setStatus(stream ? 'Screen shared' : 'Idle', stream ? 'warn' : null);
  }
  ...
  btnReset.addEventListener('click', () => {
    ...
    captureHint.textContent = stream ? 'Click any point on the chart to begin tracking it' : 'Share your screen, then click a point on the chart to track';
    clearOverlay();
    resetMetricsUI();
  ...
  function stopEverything(keepLog) {
    ...
    btnStop.disabled = true;
    btnReset.disabled = true;
    captureHint.textContent = 'Share your screen, then click a point on the chart to track';
    setStatus('Idle', null);
    clearOverlay();
    resetMetricsUI();
